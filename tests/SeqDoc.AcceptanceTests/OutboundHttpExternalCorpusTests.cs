...
public sealed class OutboundHttpExternalCorpusTests
{
    ...
    public void MalformedRunIdentityFailsClosed()
    {
        ...
        // Sibling fail-closed identity parser: data.availableTargetFrameworks (same failure surface as
        // ParseRuns - kept in this same [Fact] so the synthetic regression budget stays at 3). Parse now
        // takes the --json ROOT and does its own 'data' lookup, so a missing / non-object 'data' is
        // attributed accurately to 'data' (not to an absent availableTargetFrameworks). Each malformed
        // class raises an actionable XunitException; a well-formed array round-trips.
        static ImmutableArray<string> Tfms(string json)
        {
            using var document = JsonDocument.Parse(json);
            return OutboundHttpTargetFrameworkIdentity.Parse(document.RootElement);
        }

        Assert.Contains("'data' is absent or not an object", Assert.Throws<XunitException>(() => Tfms("[]")).Message, StringComparison.Ordinal);
        Assert.Contains("'data' is absent or not an object", Assert.Throws<XunitException>(() => Tfms("{}")).Message, StringComparison.Ordinal);
        Assert.Contains("'data' is absent or not an object", Assert.Throws<XunitException>(() => Tfms("{\"data\":\"not-an-object\"}")).Message, StringComparison.Ordinal);
        Assert.Contains("availableTargetFrameworks' is absent", Assert.Throws<XunitException>(() => Tfms("{\"data\":{}}")).Message, StringComparison.Ordinal);
        Assert.Contains("not a JSON array", Assert.Throws<XunitException>(() => Tfms("{\"data\":{\"availableTargetFrameworks\":\"net9.0\"}}")).Message, StringComparison.Ordinal);
        Assert.Contains("empty array", Assert.Throws<XunitException>(() => Tfms("{\"data\":{\"availableTargetFrameworks\":[]}}")).Message, StringComparison.Ordinal);
        Assert.Contains(
            "availableTargetFrameworks[1]' is Number, not a string",
            Assert.Throws<XunitException>(() => Tfms("{\"data\":{\"availableTargetFrameworks\":[\"net9.0\",5]}}")).Message,
            StringComparison.Ordinal);
        Assert.Contains(
            "availableTargetFrameworks[0]' is empty or whitespace",
            Assert.Throws<XunitException>(() => Tfms("{\"data\":{\"availableTargetFrameworks\":[\"  \"]}}")).Message,
            StringComparison.Ordinal);

        ImmutableArray<string> wellFormedTfms =
            Tfms("{\"data\":{\"availableTargetFrameworks\":[\"net9.0\",\"net8.0\"]}}");
        Assert.Equal(2, wellFormedTfms.Length);
        Assert.Equal("net9.0", wellFormedTfms[0]);
        Assert.Equal("net8.0", wellFormedTfms[1]);
    }
    ...
    public void ArtifactHygieneScannerCatchesInjectedVolatileMarkers()
    {
        ...
        // Machine, user-name AND user-domain leak classes injected SEPARATELY with synthetic
        // fixed-length tokens (all > the production >= 4 guard) that do not depend on the runner's real
        // identity - so all three branches are exercised on any runner.
        const string machineToken = "BUILDHOST-QA07";
        const string userToken = "svc-seqdoc-ci";
        const string domainToken = "CORP-BUILD-DOMAIN";
        Assert.Contains(
            OutboundHttpArtifactHygiene.FindVolatileMarkers(
                $"ran on {machineToken} today", forbidden, machineToken, userToken, domainToken),
            m => m.Kind == "machine-name");
        Assert.Contains(
            OutboundHttpArtifactHygiene.FindVolatileMarkers(
                $"generated by {userToken}", forbidden, machineToken, userToken, domainToken),
            m => m.Kind == "user-name");
        Assert.Contains(
            OutboundHttpArtifactHygiene.FindVolatileMarkers(
                $"on domain {domainToken}", forbidden, machineToken, userToken, domainToken),
            m => m.Kind == "user-domain");

        Assert.Empty(
            OutboundHttpArtifactHygiene.FindVolatileMarkers(
                "The method calls HttpClient.PostAsync at an outbound HTTP POST request boundary.",
                forbidden, machineToken, userToken, domainToken));

        // F1 (independent review): the Windows batch-launch shim must wrap cmd.exe as a single
        // deterministically parsed `/s /c "<"script"> <"arg"> ..."` string (one outer quote pair cmd.exe
        // strips verbatim) rather than pushing tokens onto ArgumentList, which applies C-runtime quoting
        // cmd.exe cannot parse and which misfires once a quoted npx.cmd path plus a quoted space-bearing
        // temp path put 4+ quotes on the line. Folded here as a pure harness-plumbing regression.
        string batchArgs = OutboundHttpProcessLaunch.BuildBatchArguments(
            @"C:\Program Files\nodejs\npx.cmd",
            ["--yes", "-i", @"C:\Users\ci build\Local Temp\d.mmd", "-o", "out.svg"]);
        // (c) one outer wrapper after `/s /c`, opening immediately with the individually quoted script...
        Assert.StartsWith("/s /c \"\"C:\\Program Files\\nodejs\\npx.cmd\"", batchArgs, StringComparison.Ordinal);
        // ...and closing exactly once after the last individually quoted arg.
        Assert.EndsWith("\"out.svg\"\"", batchArgs, StringComparison.Ordinal);
        // (a) a script path with a space stays intact inside the outer quotes; (b) an -i temp path with a
        // space is individually double-quoted, not left bare.
        Assert.Contains("\"C:\\Program Files\\nodejs\\npx.cmd\"", batchArgs, StringComparison.Ordinal);
        Assert.Contains("\"C:\\Users\\ci build\\Local Temp\\d.mmd\"", batchArgs, StringComparison.Ordinal);
        // An embedded double-quote in a token is escaped as `""` for cmd.exe.
        Assert.Contains("\"a\"\"b\"", OutboundHttpProcessLaunch.BuildBatchArguments("s", ["a\"b"]), StringComparison.Ordinal);
    }
        ...
        Action<string> registerOwnedTempRoot)
    {
        ...
        foreach (var file in run.Files.Where(f => f.RelativePath.EndsWith(".mmd", StringComparison.Ordinal)))
        {
            ...
            // Route npx.cmd through cmd.exe /c on Windows; launching the batch file directly with
            // UseShellExecute = false throws Win32Exception (F1).
            OutboundHttpProcessLaunch.Configure(
                startInfo,
                npx!,
                ["--yes", "@mermaid-js/mermaid-cli@11.16.0", "-i", mmdPath, "-o", svgPath]);
...
/// <summary>
/// Launch-time shim for the "must fail loud, never skip" subprocess lane. On Windows the resolved
/// <c>npx</c> is <c>npx.cmd</c> (see <c>FindOnPath</c>/<c>FindOnPathStatic</c>), and launching a
/// <c>.cmd</c>/<c>.bat</c> batch file directly with <c>UseShellExecute = false</c> throws
/// <see cref="System.ComponentModel.Win32Exception"/> - which is neither a clean pass nor the intended
/// actionable BLOCKED message.
///
/// The batch branch does NOT push tokens onto <see cref="ProcessStartInfo.ArgumentList"/>: .NET applies
/// C-runtime double-quote quoting there, and <c>cmd.exe /c</c> does not parse that form. Once 4+ quote
/// characters appear on the command line (a quoted <c>npx.cmd</c> path plus any quoted temp path with a
/// space), <c>cmd</c>'s "if the first character is a quote" rule strips the outer pair and the launch
/// breaks. Instead the batch branch sets <see cref="ProcessStartInfo.Arguments"/> to a single
/// deterministically parsed string of the form <c>/s /c "&lt;"script"&gt; &lt;"arg1"&gt; ..."</c>: with
/// <c>/s</c> plus one leading-and-trailing quote wrapping the whole <c>/c</c> argument, <c>cmd.exe</c>
/// strips exactly that outer pair and passes the inner text through verbatim (the documented
/// <c>cmd /?</c> quote-stripping rule). Every token is unconditionally double-quoted and any embedded
/// <c>"</c> is doubled to <c>""</c>. The non-batch branch (git.exe, all non-Windows) keeps
/// <c>FileName = executable</c> + per-token <see cref="ProcessStartInfo.ArgumentList"/>.
/// </summary>
internal static class OutboundHttpProcessLaunch
{
    internal static void Configure(
        ProcessStartInfo startInfo, string executable, IEnumerable<string> arguments)
    {
        ...
        if (isBatch)
        {
            ...
            startInfo.Arguments = BuildBatchArguments(executable, arguments);
            return;
        }
    ...
    /// <summary>
    /// Builds the deterministic <c>cmd.exe</c> argument string <c>/s /c "&lt;"script"&gt; &lt;"arg"&gt; ..."</c>
    /// (see the class doc). Each token is wrapped in its own double quotes; embedded <c>"</c> is escaped
    /// as <c>""</c>. The single outer quote pair after <c>/s /c</c> is the pair <c>cmd.exe</c> strips.
    /// </summary>
    internal static string BuildBatchArguments(string script, IEnumerable<string> args)
    {
        static string Quote(string token) => "\"" + token.Replace("\"", "\"\"", StringComparison.Ordinal) + "\"";

        var inner = new StringBuilder(Quote(script));
        foreach (string arg in args)
        {
            inner.Append(' ').Append(Quote(arg));
        }
...
/// <summary>
/// Fail-closed parser for the CLI <c>--json</c> <c>data.availableTargetFrameworks</c> identity array.
/// Same contract style as <see cref="OutboundHttpRunIdentity.ParseRuns"/>: throws
/// <see cref="XunitException"/> with an actionable message naming the exact defect. Takes the CLI
/// <c>--json</c> ROOT element and does its own <c>data</c> lookup, so a missing / non-object <c>data</c>
/// yields the accurate <c>'data' is absent or not an object</c> message rather than being misattributed
/// to an absent <c>availableTargetFrameworks</c> (which would happen if the caller passed <c>root</c> as
/// a stand-in for a missing <c>data</c>). Also fails closed when the array is absent / not an array /
/// empty, or any entry is non-string / empty / whitespace.
/// </summary>
internal static class OutboundHttpTargetFrameworkIdentity
{
    internal static ImmutableArray<string> Parse(JsonElement root)
    {
        ...
        {
            throw new XunitException(
                "CLI --json 'data' is absent or not an object; available target frameworks cannot be verified fail-closed.");
        }

        if (!data.TryGetProperty("availableTargetFrameworks", out var frameworks))
        {
            throw new XunitException(
                "CLI --json 'data.availableTargetFrameworks' is absent; the lane target framework cannot be verified fail-closed.");
        }

        if (frameworks.ValueKind != JsonValueKind.Array)
        {
            throw new XunitException(
                $"CLI --json 'data.availableTargetFrameworks' is {frameworks.ValueKind}, not a JSON array.");
        }
        ...
        foreach (var framework in frameworks.EnumerateArray())
        {
            ...
            {
                throw new XunitException(
                    $"CLI --json 'data.availableTargetFrameworks[{index}]' is {framework.ValueKind}, not a string.");
            }

            string text = framework.GetString() ?? string.Empty;
            if (string.IsNullOrWhiteSpace(text))
            {
                throw new XunitException(
                    $"CLI --json 'data.availableTargetFrameworks[{index}]' is empty or whitespace.");
            }
        ...
        {
            throw new XunitException(
                "CLI --json 'data.availableTargetFrameworks' is an empty array; no target framework identity was emitted.");
        }
...
public sealed class OutboundHttpExternalCorpusFixture : IAsyncLifetime
{
    ...
    private async Task<OutboundHttpLaneRun> RunCliAsync(string solution, string laneRoot, string configPath)
    {
        ...
        bool hasData = root.TryGetProperty("data", out var data) && data.ValueKind == JsonValueKind.Object;
        if (hasData && data.TryGetProperty("toolchainVersion", out var tv) && tv.ValueKind == JsonValueKind.String)
        {
            toolchainVersion = tv.GetString();
        }

        // Fail closed on target-framework identity (same fail-closed style as ParseRuns): Parse takes the
        // --json ROOT and does its own 'data' lookup, so a missing/non-object 'data', an absent /
        // non-array / empty 'availableTargetFrameworks', or a non-string / empty / whitespace entry each
        // raises the accurately attributed actionable XunitException here rather than being silently
        // tolerated and left to a generic downstream Assert.Contains.
        ImmutableArray<string> availableTfms =
            OutboundHttpTargetFrameworkIdentity.Parse(root);
        ...
        params string[] arguments)
    {
        ...
        // F1: a resolved .cmd/.bat (Windows npx.cmd) is routed through cmd.exe /c; launching it
        // directly with UseShellExecute = false throws Win32Exception. This is the shared seam for
        // ReadMermaidCliVersion's npx call and every git invocation (git.exe is unaffected).
        OutboundHttpProcessLaunch.Configure(startInfo, executable, arguments);
...
internal static class OutboundHttpArtifactHygiene
{
    ...
    // Production call site: the real machine / user / user-domain identity strings.
    ...
    // Test seam (G2/F3): the three machine/user/domain identity tokens are injected so a regression can
    // exercise EACH leak branch with synthetic fixed-length tokens instead of depending on the runner's
    // real (possibly short) identity strings and the production short-token false-positive guard.
    internal static IReadOnlyList<(string Marker, string Kind)> FindVolatileMarkers(
        string text,
        ...
        string userDomainName)
    {
        ...
        {
            // Production keeps a short-token false-positive guard (>= 4). The regression injects
            // synthetic fixed-length tokens so it never relies on that guard.
